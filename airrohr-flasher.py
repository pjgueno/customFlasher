...
class MainWindow(QtWidgets.QMainWindow, mainwindow.Ui_MainWindow):
    ...
    def on_wifiButton_clicked(self):
        ...
        jsonTest = jsonPart1 + ssid + jsonPart2 + pw + jsonPart3 + "airRohr-"+ str(esp.chip_id()) + jsonPart4
        
        if not self.is_json(jsonTest):
            self.statusbar.showMessage(self.tr("Created invalid json."))
            return
        else:
            self.jsonFinal = jsonPart1 + ssid + jsonPart2 + pw + jsonPart3 + "airRohr-"+ str(esp.chip_id()) + jsonPart4
